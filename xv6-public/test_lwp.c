...
// Test behavior when a lwp is created in a lwp
int lwpinlwptest(void);

// Test behavior when a lwp is created recursively in a lwp
int recursivelwptest(void);

volatile int gcnt;
int gpipe[2];

#define MAKE_TEST(test_func) {#test_func, test_func}

struct {
  const char* test_name;
  int (*testfunc)(void);
} test_cases[] = {
  MAKE_TEST(racingtest),
  MAKE_TEST(basictest),
  MAKE_TEST(jointest1),
  MAKE_TEST(jointest2),
  MAKE_TEST(stresstest),
  MAKE_TEST(exittest1),
  MAKE_TEST(exittest2),
  // MAKE_TEST(forktest),      // Not properly implemented
  MAKE_TEST(exectest),
  MAKE_TEST(sbrktest),
  MAKE_TEST(killtest),
  MAKE_TEST(pipetest),
  MAKE_TEST(sleeptest),
  MAKE_TEST(stridetest),
  MAKE_TEST(lwpinlwptest),
  // MAKE_TEST(recursivelwptest), // Not properly implemented
};
...
main(int argc, char *argv[])
{
  ...
  int pid;
  int start = 0;
  int end = sizeof(test_cases)/sizeof(test_cases[0])-1;
  if (argc >= 2)
    start = atoi(argv[1]);
  if (argc >= 3)
    end = atoi(argv[2]);
...
lwpinlwpthreadmain(void *arg)
{
  ...
  if (thread_create(&t, lwpinlwpthreadmain, (void*)((int)arg - 1)) != 0){
    printf(1, "panic at lwpinlwpthreadmain\n");
    thread_exit((void*)-1);
  }
  if (thread_join(t, &retval) != 0){
    printf(1, "panic at thread_join\n");
    thread_exit((void*)-1);
  }

  thread_exit((void*)(retval == 0 ? 0 : -1));
  return 0;
}
...
lwpinlwptest(void)
{
  ...
  if (thread_create(&thread, lwpinlwpthreadmain, (void*)NUM_THREAD) != 0){
    printf(1, "panic at thread_create\n");
    return -1;
  }
  if (thread_join(thread, &retval) != 0){
    printf(1, "panic at thread_join\n");
    return -1;
  }
...
void*
recursivelwpthreadmain(void *arg)
{
  thread_t t;
  void* retval;
  
  if (thread_create(&t, recursivelwpthreadmain, 0) != 0){
    printf(1, "panic at recursivelwpthreadmain\n");
    thread_exit((void*)-1);
  }
  if (thread_join(t, &retval) != 0){
    printf(1, "panic at thread_join\n");
...
int
recursivelwptest(void)
{
  thread_t thread;
  void *retval;

  if (thread_create(&thread, recursivelwpthreadmain, 0) != 0){
    printf(1, "panic at thread_create\n");
    return -1;
  }
  if (thread_join(thread, &retval) != 0){
